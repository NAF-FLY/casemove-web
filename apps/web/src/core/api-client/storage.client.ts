...
// Withdraw types (same structure as deposit)
export type StorageWithdrawResult = StorageDepositResult;
export type StorageWithdrawResponse = StorageDepositResponse;

export async function withdrawFromStorage(
  storageId: string,
  itemIds: string[]
): Promise<StorageWithdrawResponse> {
  const response = await fetchWithAuth(`/api/storage/${storageId}/withdraw`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ itemIds })
  });
  ...
  if (!response.ok) {
    const errorBody = await response.json().catch(() => ({}));
    throw new Error(errorBody.message || "Failed to withdraw items");
  }
